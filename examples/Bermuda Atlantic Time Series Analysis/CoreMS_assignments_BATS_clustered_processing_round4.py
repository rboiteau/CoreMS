...
results_clustered=pd.read_csv(file_location+'BATS_clustered_results_round4.csv')
...
results_clustered.cluster[results_clustered.cluster==4]='3.SRDOM'
...
sns.kdeplot(x='Dispersity',hue='cluster', data=results_clustered, ax=axs['e'], common_norm=False,legend=False)
...
axs['d'].set_title('d', fontweight='bold', loc='left')
axs['d'].set_xlabel("$\it{m/z}$")
sns.violinplot(x='Dispersity',y='cluster', data=results_clustered, ax=axs['e'], common_norm=False,legend=False)
axs['e'].set_title('e', fontweight='bold', loc='left')
