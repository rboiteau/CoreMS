...
for file in MSfiles:
    ...
    allresults.to_csv(file_location+file.replace(dfiletype,'_mf_assignments.csv'))
    ...
    fig.savefig(file_location+file.replace(dfiletype,'_errorplot.pdf'),dpi=300,format='pdf')
...
fig.savefig(file_location+savefile.replace('.csv','_assigment_plot.eps'),dpi=300,format='eps')
